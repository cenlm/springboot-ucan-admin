 ...
 * @date:2023/02/13
 * 已自定义节点新增、修改、删除操作
 * 已自定义的方法：cancelAllChecked、disableCheckbox
 */
    ...
    layui.each(data, function(index, item){
      ...
      ,entryDiv = $(['<div data-id="'+ item.id +'" class="layui-tree-set'+ (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") +'">'
        ,'<div class="layui-tree-entry">'
          ,'<div class="layui-tree-main">'
            ...
            ,function(){
              if(options.showLine){
                if(hasChild){//item.icon 字体图标可动态配置，数据可从后台获取
                  return '<span class="layui-tree-iconClick layui-tree-icon"><i class="layui-icon '+ (item.spread ? "layui-icon-subtraction" : "layui-icon-addition") +'"></i></span>'+'<span style="color:#c0c4cc;margin-left:-5px;margin-right:5px;vertical-align: middle;line-height: 20px;height:20px;position: relative;display: inline-block;"><i class="layui-icon '+item.icon+'" style="font-size:16px;"></i></span>';
                }else{//item.icon 字体图标可动态配置，数据可从后台获取
                  return '<span class="layui-tree-iconClick" style="margin-right:5px;"><i class="layui-icon '+item.icon+'" style="font-size:16px;"></i></span>';
                };
              }else{
                return '<span class="layui-tree-iconClick"><i class="layui-tree-iconArrow '+ (hasChild ? "": HIDE) +'"></i></span>'+'<span style="color:#c0c4cc;margin-left:-5px;margin-right:5px;vertical-align: middle;line-height: 20px;height:20px;position: relative;display: inline-block;"><i class="layui-icon '+item.icon+'" style="font-size:16px;"></i></span>';
              };
            }()
      ...
      ,function(){
        ...
        var editIcon = {
          ...
          ,update: '<i class="layui-icon layui-icon-edit" data-type="update" title="更新"></i>'
...
//            elemMain.find('.'+ICON_CLICK).children('.layui-icon').addClass(ICON_ADD).removeClass('layui-icon-set-fill');
//            //若未开启连接线，显示箭头
//          }else{
...
//              prevDiv.find('.'+ICON_CLICK).children('.layui-icon').removeClass(ICON_SUB).addClass('layui-icon-set-fill');
//              //父节点所在层添加延伸线
...
//        });
  ...
  //取消所有节点的选中
  Class.prototype.cancelAllChecked = function(){
      var that = this;
   ...
   //让复选框不可点击
  //parentId   当前节点的父节点Id，如果等于-1，则说明当前节点时根节点
  //inputIdArr 传入的父角色拥有的所有权限ID
  //节点不包含在 inputIdArr 里的节点，设置为禁用状态
  Class.prototype.disableCheckbox = function(parentId,inputIdArr){
      ...
      that.elem.find('.'+ELEM_SET).each(function(i, item){
          var thisId = $(this).data('id')
              ,input = $(item).children('.'+ELEM_ENTRY).find('input[same="layuiTreeCheck"]')
              ,reInput = input.next();
          if(inputIdArr.length>0) {//父角色有权限才进行操作
              if(inputIdArr.indexOf(thisId)==-1) {//当前节点id 不等于父节点的权限ID
                  $(input[0]).prop("disabled", true);//禁用指定的复选框
                  that.renderForm('checkbox');
                  //文字变为禁用灰色颜色
                  $(item).children('.'+ELEM_ENTRY).find('span[class="layui-tree-txt"]').addClass("layui-disabled");
                //渲染checkbox 否则只会在input 添加disabled属性，
		//不会在checkbox同级节点添加div 并新增layui-disabled样式
                  
              }else {
        	  $(input[0]).prop("disabled", false);//移除禁用
        	  that.renderForm('checkbox');
        	  $(item).children('.'+ELEM_ENTRY).find('span[class="layui-tree-txt layui-disabled"]').removeClass("layui-disabled");
                   
              }
          }else {
              if(parseInt(parentId)==-1) {//初始状态：所有角色都没有分配权限时，只有（根角色）超级管理员可以进行权限选择
...
//设置选中节点（id:树实例的索引id ）
  tree.cancelAllChecked = function(treeId){
      var that = thisModule.that[treeId];
      return that.cancelAllChecked();
  };
  
  //禁用指定节点的复选框
  tree.disableCheckbox = function(treeId,parentId,inputIdsArr){
      var that = thisModule.that[treeId];
      return that.disableCheckbox(parentId,inputIdsArr);
  };
